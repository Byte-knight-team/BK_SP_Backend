...
public class KitchenServiceImpl implements KitchenService {
        ...
        public KitchenDashboardStatsDTO getKitchenDashboardStats(String userEmail) {
                ...
                long pending = orderRepository.countByBranchIdAndStatusAndCreatedAtAfter(branchId, OrderStatus.PENDING,
                                startOfToday);
                long preparing = orderRepository.countByBranchIdAndStatusAndCreatedAtAfter(branchId,
                                OrderStatus.PREPARING, startOfToday);
                long completed = orderRepository.countByBranchIdAndStatusAndCreatedAtAfter(branchId,
        ...
        public List<PopularMealDTO> getMostPopularMealsInLast7Days(String userEmail) {
                ...
                for (Object[] row : topMeals) {
                        ...
                        // SQL SUM() typically returns Long/BigInteger. Direct cast Long to Integer will
                        // fail.
                        // Using 'Number' (the parent class) prevents crashes by safely handling any
                        // numeric type.
        ...
        public List<InventoryDetailsDTO> getInventoryAlerts(String userEmail) {
                ...
                List<InventoryItem> items = inventoryItemRepository.findByBranchId(branchId); // no need to write native
                                                                                              // query
        ...
        @Override
        public List<OrderCardDetailsDTO> getOrdersByStatus(OrderStatus status, String userEmail) { // Added email
                                                                                                   // parameter
                ...
                List<Order> orders = orderRepository.findByBranchIdAndStatusAndStatusUpdatedAtAfter(branchId, status,
                                startOfToday, sort);
                ...
                for (Order order : orders) {
                        ...
                        // add OrderCardDetailsDTO list to the array list using .add() method and call
                        // the all args constructor of OrderCardDetailsDTO
        ...
        public List<InventoryDetailsDTO> getAllInventoryItems(String userEmail) {
                ...
                for (InventoryItem item : items) {
                        ...
                        dtoList.add(new InventoryDetailsDTO(
                                        ...
                                        current,
                                        item.getUnit(),
                                        warningLevel // No alert level needed here, so set as NORMAL or you can modify
                                                     // the DTO to make it optional
                        ));
                }
                return dtoList;
        }
        ...
        public void createRequest(InventoryRequestDTO requestDTO, String userEmail) {
                ...
                // get the Staff profile to find their branch to identify the specific branch
                // making the request
                Staff staff = staffRepository.findByUser(user)
                                .orElseThrow(() -> new RuntimeException("Staff profile not found"));
        ...
        public void updateInventoryStock(UpdateStockDTO updateDTO, String userEmail) {
                ...
                // save it back (in the entity already has @PreUpdate to automatically set the
                // lastUpdated time)
        ...
        public OrderDetailsDTO getOrderDetails(Long orderId, String userEmail) {
                ...
                for (OrderItem item : order.getItems()) { // Loop through each item in the order
                        ...
                        OrderItemDetailsDTO dto = new OrderItemDetailsDTO(
                                        ...
                                        item.getAssignedChef() != null ? item.getAssignedChef().getUser().getFullName()
                                                        : "Not Assigned");
        ...
        // get all available Line chefs from the same branch of the logged-in Chief Chef
        // to assign them to prepare meals
        @Override
        public List<ChefAssignDTO> getAvailableChefsForAssignment(String userEmail) {
                ...
                // 1. Same Branch: Only chefs working at the same location as the logged-in
                // Chief Chef.
                // 2. Today's Attendance: Must have a recorded attendance entry for the current
                // date.
                // 3. On-Duty Only: The chef must be currently clocked-in (Attendance Status:
                // ON_DUTY).
                // 4. Working Status: Must be either currently 'AVAILABLE' (idle) or 'COOKING'
                // (already assigned to a meal).
        ...
        public List<ChefDetailsDTO> getChefDetailsToday(String chiefChefEmail) {
                ...
                for (Staff chef : lineChefs) {
                        ...
                        Optional<ChefAttendance> attendance = chefAttendanceRepository
                                        .findByStaffIdAndAttendanceDate(chef.getId(), LocalDate.now());
                        ...
                        // If there is an attendance record, use its status (e.g., AVAILABLE, COOKING,
                        // UNAVAILABLE)
        ...
        public void checkInChef(Long chefId) {
                ...
                Optional<ChefAttendance> existingRecord = chefAttendanceRepository
                                .findByStaffIdAndAttendanceDate(chefId, LocalDate.now());
                ...
                if (existingRecord.isPresent()) {
                        ...
                        if (record.getAttendanceStatus() == ChefAttendanceStatus.OFF_DUTY) {
                                throw new RuntimeException(
                                                "Cannot check-in for today. Chef has already completed their shift for today!");
                        }
                }
                ...
                attendance.setAttendanceStatus(ChefAttendanceStatus.ON_DUTY); // now the chef is clocked in and ready to
                                                                              // work
        ...
        // check out a chef and update the attendance record with clock-out time,
        // attendance status, and work status
        @Override
        @Transactional
        public void checkOutChef(Long chefId) {
                ...
                ChefAttendance attendance = chefAttendanceRepository
                                .findByStaffIdAndAttendanceDate(chefId, LocalDate.now())
                                .orElseThrow(() -> new RuntimeException(
                                                "No check-in record found for this chef today!"));
        ...
        public void updateChefWorkStatus(Long chefId, ChefWorkStatus newStatus) {
                ...
                ChefAttendance attendance = chefAttendanceRepository
                                .findByStaffIdAndAttendanceDate(chefId, LocalDate.now())
                                .orElseThrow(() -> new RuntimeException(
                                                "Chef is not checked in today!. Cannot update status."));
                ...
                // I do not provide this option in the frontend. but this is just an extra
                // safety check.
        ...
        // hold an order and update the order status to ON_HOLD with a reason, also
        ...
        // update meal status, order status, and chef work status when start preparing a
        // meal
        @Override
        @Transactional
        ...
        public List<ActiveAlertDTO> getActiveAlerts(String userEmail) {
                ...
                for (KitchenAlert alert : alerts) {
                        ...
                        // Calculate the time difference (in minutes) between alert creation and current
                        // time
                        long minutes = java.time.Duration.between(alert.getCreatedAt(), LocalDateTime.now())
                                        .toMinutes();

                        // If less than 60 minutes, show in minutes (e.g., 10m). Otherwise, show in
                        // hours (e.g., 2h).
        ...
        // get details for chef dashboard stats (total chefs, on-duty, off-duty,
        // available)
        @Override
        public ChefDashboardStatsDTO getChefDashboardStats(String userEmail) {
                ...
                long onDuty = chefAttendanceRepository.countChefsByAttendanceStatusToday(today, branchId,
                                ChefAttendanceStatus.ON_DUTY);
                long offDuty = chefAttendanceRepository.countChefsByAttendanceStatusToday(today, branchId,
                                ChefAttendanceStatus.OFF_DUTY);
                long available = chefAttendanceRepository.countChefsByWorkStatusToday(today, branchId,
