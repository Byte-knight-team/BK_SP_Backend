...
public class LineChefServiceImpl implements LineChefService {
        ...
        public List<LineChefItemDTO> getMyItems(String userEmail) {
                ...
                List<OrderItem> todayDoneItems = orderItemRepository.findByAssignedLineChefIdAndStatusIn(
                                lineChef.getId(),
                                List.of(OrderItemStatus.READY, OrderItemStatus.SERVED)).stream()
        ...
        public void startItem(Long itemId, String userEmail) {
                ...
                if (branchId != null) {
                        webSocketNotificationService.broadcastKitchenItemUpdate(branchId, order.getId(),
                                        order.getOrderNumber(), item.getItemName(), "PREPARING",
                                        order.getStatus().name(), order.getOrderType().name());
                }
        }
        ...
        public void completeItem(Long itemId, String userEmail) {
                ...
                if (branchId != null) {
                        webSocketNotificationService.broadcastKitchenItemUpdate(branchId, order.getId(),
                                        order.getOrderNumber(), item.getItemName(), "READY", order.getStatus().name(),
                                        order.getOrderType().name());
                }
