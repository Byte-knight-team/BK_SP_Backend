...
public class ManagerAnalyticsServiceImpl implements ManagerAnalyticsService {
        ...
        public AnalyticsSummaryDTO getBranchAnalyticsSummary(Long branchId, Long userId, LocalDate startDate,
                        LocalDate endDate) {
                ...
                BigDecimal totalInventoryValue = items.stream()
                                .map(item -> {
                                        BigDecimal qty = item.getQuantity() != null ? item.getQuantity()
                                                        : BigDecimal.ZERO;
                                        BigDecimal price = item.getUnitPrice() != null ? item.getUnitPrice()
                                                        : BigDecimal.ZERO;
                ...
                List<Long> orderIds = orders.stream().map(com.ByteKnights.com.resturarent_system.entity.Order::getId)
                                .collect(Collectors.toList());
                ...
                if (!orderIds.isEmpty()) {
                        ...
                        for (OrderItem oi : orderItems) {
                                ...
                                dto.setRevenue(dto.getRevenue()
                                                .add(oi.getSubtotal() != null ? oi.getSubtotal() : BigDecimal.ZERO));
