...
public class OrderServiceImpl implements OrderService {
    ...
    public OrderResponse placeCustomerOrder(String userIdentifier, PlaceOrderRequest request) {
        ...
        if (OrderType.QR.name().equals(request.getOrderType())) {
            if (request.getTableId() == null) throw new CheckoutException(HttpStatus.BAD_REQUEST, "Table ID required for Dine-In");
            table = tableRepository.findById(request.getTableId()).orElseThrow(() -> new ResourceNotFoundException("Table not found"));
        }
        ...
        CheckoutCalculateResponse trustedMath = checkoutService.calculateOrderTotals(userIdentifier, calcRequest);
        ...
        if (trustedMath.getLoyaltyPointsEarnedThisOrder() > 0) {
            customer.setLoyaltyPoints(customer.getLoyaltyPoints() + trustedMath.getLoyaltyPointsEarnedThisOrder());
